...
function Chip({ tone = "neutral", children }: { tone?: "ok" | "bad" | "warn" | "accent" | "neutral"; children: ReactNode }) {
  const tones: Record<string, string> = {
    ok: "bg-[var(--ok)]/15 text-[var(--ok)] border-[var(--ok)]/30",
    bad: "bg-[var(--bad)]/10 text-[var(--bad)] border-[var(--bad)]/30",
    warn: "bg-[var(--warn)]/15 text-[var(--warn)] border-[var(--warn)]/30",
    accent: "bg-[var(--accent-soft)] text-[var(--accent-strong)] border-[var(--accent)]/30",
    neutral: "bg-[var(--bg-sunken)] text-[var(--ink-soft)] border-[var(--line-strong)]",
  };
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${tones[tone]}`}>
      {children}
    </span>
  );
}
...
function RawJson({ payload }: { payload: Unknown }) {
  return (
    <details className="mt-3 rounded-lg border border-[var(--line)]/70 bg-[var(--bg-sunken)]/40">
      <summary className="cursor-pointer select-none px-3 py-1.5 text-[11px] font-bold text-[var(--ink-faint)] hover:text-[var(--ink-soft)]">
        Raw JSON
      </summary>
      <pre className="max-h-80 overflow-auto border-t border-[var(--line)]/70 p-3 font-mono text-[11px] leading-relaxed text-[var(--ink-soft)]">
...
function TextBlock({ text, maxH }: { text: string; maxH?: string }) {
  return (
    <div
      className={`whitespace-pre-wrap break-words rounded-lg border border-[var(--line)]/70 bg-[var(--bg-sunken)]/40 p-3 text-sm leading-relaxed text-[var(--ink-soft)] ${maxH ? "overflow-auto" : ""}`}
...
function Stat({ label, value, strong }: { label: string; value: string | number; strong?: boolean }) {
  return (
    <div className="flex min-w-20 flex-col items-center rounded-lg border border-[var(--line)]/70 bg-[var(--bg)] px-3 py-2">
      <span className={`text-lg font-extrabold tabular-nums ${strong ? "text-[var(--accent-strong)]" : "text-[var(--ink)]"}`}>{value}</span>
      <span className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-[var(--ink-faint)]">{label}</span>
    </div>
  );
}
...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="mt-4 first:mt-0">
      <p className="mb-2 text-[11px] font-extrabold uppercase tracking-widest text-[var(--ink-faint)]">{title}</p>
      {children}
    </div>
  );
}
...
function TriageView({ p }: { p: Unknown }) {
  ...
  const total = asNum(p.total_failures) ?? verdicts.reduce((acc, v) => acc + (asNum(asObj(v)?.count) ?? 0), 0);
  return (
    <div>
      <div className="flex items-center gap-2">
...
function CodegenView({ p }: { p: Unknown }) {
  ...
  const treeEntries = filesTree ? Object.values(filesTree).flatMap((v) => (Array.isArray(v) ? v : [])) : [];
  const grounding = asObj(p.grounding_report);
  const counts = asObj(grounding?.counts) ?? {};
  const fileEntries = Object.keys(bundle).length ? Object.entries(bundle) : treeEntries.map((t) => [asStr(asObj(t)?.name), asStr(asObj(t)?.content)] as [string, string]);
              ...
              <div key={dir}>
                ...
                <ul className="mt-1 space-y-1">
                  {files.map(([name, content]) => (
                    <li key={dir + "/" + name} className="rounded-md border border-[var(--line)]/60 bg-[var(--bg)]">
                      <details>
                        <summary className="flex cursor-pointer items-center gap-2 px-2.5 py-1.5 text-xs">
                          <span className="text-[var(--accent-strong)]">▸</span>
                          <span className="font-mono font-semibold text-[var(--ink)]">{name}</span>
                          <span className="ml-auto text-[10px] tabular-nums text-[var(--ink-faint)]">{(content.length / 1024).toFixed(1)} KB</span>
                        </summary>
                        <pre className="max-h-64 overflow-auto border-t border-[var(--line)]/60 bg-[var(--bg-sunken)]/60 p-2.5 font-mono text-[11px] leading-relaxed text-[var(--ink-soft)]">
                          {content}
                        </pre>
                      </details>
                    </li>
...
function GenericView({ p }: { p: Unknown }) {
  ...
  const arrays = Object.entries(p).filter(([, v]) => Array.isArray(v));
  const objects = Object.entries(p).filter(([, v]) => v != null && typeof v === "object" && !Array.isArray(v));
  return (
    <div>
      {scalars.length > 0 && (
          ...
          <p className="text-xs text-[var(--ink-soft)]">
            {objects.map(([k, o]) => (
              <span key={k} className="mr-3">
                <span className="font-mono font-bold text-[var(--accent-strong)]">{k}</span> · {Object.keys(o as Unknown).join(", ")}
              </span>
            ))}
          </p>
        </Section>
...
export function ArtifactView({ payload }: { payload: Unknown }) {
  ...
  if (asStr(payload.verdict) && (asStr(payload.reason) || asObj(payload.evidence))) {
    return <ReleaseView p={payload} />;
  }
  // requirement doctor — diagnosis + optional enhanced rewrite
  if (asObj(payload.diagnosis) || ("quality_score" in payload && asArr(payload.findings))) {
    return <DoctorView p={payload} />;
  }
  // generated POM framework
  if (asObj(payload.bundle) || asObj(payload.files) || ("files" in payload && asArr(payload.files))) {
    return <CodegenView p={payload} />;
  }
  // test case generation
  if (asArr(payload.cases)) {
    return <CasesView p={payload} />;
  }
  // triage — root-cause clusters
  if (asArr(payload.clusters) || asArr(payload.verdicts)) {
    return <TriageView p={payload} />;
  }
  // executor results
  if (asArr(payload.results)) {
    return <RunResultsView p={payload} />;
  }
  // intake requirement
  if ("text" in payload && "word_count" in payload && asNum(payload.word_count) != null) {
    return <RequirementView p={payload} />;
  }
  return <GenericView p={payload} />;
}
